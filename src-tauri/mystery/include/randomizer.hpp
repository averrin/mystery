...
public:
  ...
  template <typename T> auto getGen(std::string key) {
    return std::any_cast<std::shared_ptr<T>>(generators.at(key));
  }

  template <typename T> auto reg(std::string key) {
  ...
  template <typename T> auto get(std::string key, bool uniq = false) {
